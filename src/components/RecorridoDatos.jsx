...
export default function RecorridoDatos() {
      ...
      const waypoints =
        simType === "sata"
          ? [
              ...
              {
                x: nodes.chipset.x + nodes.chipset.w / 2,
                y: nodes.chipset.y + 30,
              },
  ...
  const animate = (time) => {
    ...
    const fracSata =
      type === "sata" || type === "both"
        ? Math.min(elapsed / DURATION_SATA, 1)
        : 0;
    const fracNvme =
      type === "nvme" || type === "both"
        ? Math.min(elapsed / DURATION_NVME, 1)
        : 0;
    ...
    if (type === "sata") {
      ...
      else if (fracSata < 0.5)
        setStatusText("Cuello de botella en Controlador SATA (1 Cola)...");
      else if (fracSata < 0.75)
        setStatusText("Transfiriendo a RAM y consolidando datos (DMA)...");
      else setStatusText("CPU procesando datos a través del bus...");
    } else if (type === "nvme") {
      if (fracNvme < 0.25)
        setStatusText("Extracción en paralelo de Memoria Flash...");
      else if (fracNvme < 0.5) setStatusText("Pasando por Bus PCI Express...");
      else if (fracNvme < 0.75)
        setStatusText(
          "Copiado directo a RAM y consolidación (DMA Integrado)...",
        );
      else setStatusText("CPU procesando datos a través del bus...");
    } else if (type === "both") {
      if (fracNvme < 1)
        setStatusText("Competición: NVMe liderando, HDD buscando sectores...");
      else if (fracSata < 1)
        setStatusText("NVMe finalizó. HDD sufre cuello de botella en AHCI...");
    ...
    if (!isDone) {
      ...
      if (type === "sata") setStatusText("Lectura completada (Alta latencia)");
        ...
        setStatusText("Lectura completada (Ultrarrápida)");
    ...
    <div className="flex flex-col xl:flex-row gap-6 w-full text-slate-200">
      ...
      <div className="flex-[2] bg-[#141c25] border border-slate-700 rounded-xl p-6 shadow-xl flex flex-col gap-6">
        ...
        <div className="text-center p-3 bg-[#0b1016] border border-slate-800 rounded-lg">
          ...
          <span
            className={`text-sm font-bold ml-2 ${currentSimType === "nvme" ? "text-blue-400" : currentSimType === "sata" ? "text-cyan-400" : currentSimType === "both" ? "text-purple-400" : "text-slate-300"}`}
          >
            {statusText}
          </span>
        </div>
        ...
        <div className="space-y-5">
          <div className="bg-[#0b1016] p-4 rounded-lg border border-slate-800 border-l-4 border-l-cyan-500">
            ...
            <p className="text-xs text-slate-300 mt-1 leading-relaxed text-justify">
              Los datos viajan desde el disco hacia el{" "}
              <strong>Controlador SATA</strong>. Debido a la naturaleza mecánica
              y a la única cola de comandos del protocolo AHCI, se genera un
              cuello de botella antes de pasar por la memoria RAM hacia el
              procesador.
            </p>
          </div>
          ...
          <div className="bg-[#0b1016] p-4 rounded-lg border border-slate-800 border-l-4 border-l-blue-500">
            ...
            <p className="text-xs text-slate-300 mt-1 leading-relaxed text-justify">
              El protocolo NVMe omite el controlador heredado. Se comunica de
              manera directa usando carriles <strong>PCI Express</strong>,
              permitiendo un flujo paralelo ininterrumpido hacia la memoria RAM
              y la CPU, logrando velocidades exponenciales.
            </p>
          </div>

          <div className="p-4 bg-[#0b1016] border border-slate-700 rounded-lg shadow-inner border-l-4 border-l-purple-500">
            <h4 className="text-purple-400 font-semibold text-sm border-b border-slate-800 pb-2 mb-3">
              Integración DMA (Direct Memory Access)
            </h4>
            <p className="text-xs text-slate-300 leading-relaxed text-justify">
              Ambos recorridos utilizan DMA. Al transferir información, el
              controlador toma el control del bus y envía los datos directamente
              a la <strong>Memoria RAM</strong> sin que la CPU intervenga byte
              por byte. La CPU se activa únicamente al recibir la interrupción
              de finalización, mejorando la eficiencia general.
            </p>
          </div>
        </div>
      </div>
