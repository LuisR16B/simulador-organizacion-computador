...
export default function ChipFlash() {
  ...
  const [metrics, setMetrics] = useState({ seek: '0.0 ms', latency: '0.00 ms' });
    ...
    <div className="w-full flex flex-col">
      <div className="flex flex-col xl:flex-row gap-6 mb-6">
        ...
        <div className="flex-1 bg-[#141c25] border border-slate-700 p-6 rounded-xl flex flex-col items-center relative overflow-hidden shadow-lg">
          ...
          <div className="relative w-48 h-48 flex items-center justify-center mt-2 mb-8">
            <motion.div 
              ...
              animate={{ rotate: isSimulating && !hddStatus.includes('¡Dato') ? 1440 : 0 }}
          ...
          <div className="w-full bg-[#0b1016] p-4 rounded-lg text-center border border-slate-800 shadow-inner mt-auto">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest block mb-2">Estado Físico:</span>
            <span className={`text-sm font-mono font-bold ${hddStatus.includes('¡Dato') ? 'text-cyan-400' : 'text-amber-400'}`}>
              {hddStatus}
            </span>
          </div>
        </div>
        ...
        <div className="flex-[2] bg-blue-950/10 border border-blue-900/30 rounded-xl p-6 relative overflow-hidden flex flex-col shadow-inner">
          ...
          <div className="absolute inset-0 opacity-20 pointer-events-none" style={{ backgroundImage: 'radial-gradient(#3b82f6 1px, transparent 1px)', backgroundSize: '20px 20px' }}></div>

          <div className="flex items-center justify-between w-full h-full relative z-10 px-4">
            <div className="flex flex-col items-center">
              <div className={`w-20 h-24 bg-[#0b1016] border-2 rounded-md flex items-center justify-center shadow-lg transition-colors duration-300 ${isSimulating && !ssdStatus.includes('¡Dato') ? 'border-blue-400 shadow-[0_0_20px_rgba(59,130,246,0.5)]' : 'border-slate-700'}`}>
                <Microchip size={36} className={isSimulating && !ssdStatus.includes('¡Dato') ? 'text-blue-400' : 'text-slate-500'} />
              </div>
              <span className="text-xs text-blue-500/70 font-bold mt-2 bg-[#0b1016] px-2 py-1 rounded border border-blue-900/30">Controlador</span>
            </div>

            <div className="flex-1 h-40 relative mx-4">
              {chipsNand.map((chipId) => (
                <div key={`bus-${chipId}`} className="absolute w-full flex items-center" style={{ top: `${(chipId * 25) + 12}%` }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {chipsNand.map((chipId) => (
                <div key={`chip-${chipId}`} className="bg-[#0b1016] border border-slate-700 p-2 rounded flex flex-col items-center relative shadow-lg">
                  <span className="text-[10px] text-slate-500 font-mono mb-2">NAND {chipId}</span>
                  <div className="grid grid-cols-2 gap-1.5">
                    {blocksPerChip.map((blockId) => {
                      const isTarget = activeChip === chipId && activeBlock === blockId;
                      return (
                        <motion.div 
                          ...
                          animate={{ 
                            ...
                            boxShadow: isTarget ? '0 0 15px 2px rgba(59,130,246,0.8)' : '0 0 0 rgba(0,0,0,0)'
                          }}
                        />
                      );
                    })}
          ...
          <div className="w-full bg-[#0b1016] p-4 rounded-lg text-center border border-slate-800 shadow-inner mt-8 relative z-20">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest block mb-2">Estado Lógico:</span>
            <span className={`text-sm font-mono font-bold ${ssdStatus.includes('¡Dato') ? 'text-blue-400' : 'text-blue-500/70'}`}>
              {ssdStatus}
            </span>
          </div>
        </div>
      ...
      <div className="flex gap-4 mb-6">
        ...
        >
          <Zap size={20} className={isSimulating ? 'animate-pulse text-blue-300' : ''} />
          {isSimulating ? 'Procesando lectura simultánea...' : 'Iniciar Comparativa de Lectura'}
        </button>

        {!isSimulating && (activeChip !== null || hddStatus.includes('¡Dato')) && (
          <button 
            onClick={resetear}
      ...
      <div className="w-full bg-[#141c25] border border-slate-700 rounded-xl p-6 shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#0b1016] border border-slate-800 p-4 rounded-lg flex flex-col relative overflow-hidden shadow-inner">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-3">Tiempo de Búsqueda (Seek Time)</span>
            <div className="flex justify-between items-end mb-2">
              <span className="text-xs text-slate-400">HDD Mecánico:</span>
              <span className="text-sm font-mono text-red-400/80">~8.50 ms</span>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-xs text-blue-400 font-bold">SSD Flash:</span>
              <span className="text-xl font-mono text-blue-400">{metrics.seek}</span>
            </div>
            <p className="mt-4 text-xs text-slate-400 border-t border-slate-800 pt-3 leading-relaxed text-justify">
              <strong>Ausencia de partes mecánicas:</strong> Al carecer de un brazo móvil, el SSD no necesita desplazarse físicamente a ninguna pista. Su velocidad de acceso es electrónicamente inmediata.
            </p>
          </div>

          <div className="bg-[#0b1016] border border-slate-800 p-4 rounded-lg flex flex-col relative overflow-hidden shadow-inner">
            <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-3">Latencia Rotacional</span>
            <div className="flex justify-between items-end mb-2">
              <span className="text-xs text-slate-400">HDD (7200 RPM):</span>
              <span className="text-sm font-mono text-red-400/80">~4.16 ms</span>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-xs text-blue-400 font-bold">SSD Flash:</span>
              <span className="text-xl font-mono text-blue-400">{metrics.latency}</span>
            </div>
            <p className="mt-4 text-xs text-slate-400 border-t border-slate-800 pt-3 leading-relaxed text-justify">
              <strong>Menor latencia:</strong> El HDD debe esperar a que el sector correcto gire hasta el cabezal. El SSD utiliza una matriz de memoria Flash NAND para acceder directamente a la celda.
            </p>
          </div>
        </div>
      </div>
