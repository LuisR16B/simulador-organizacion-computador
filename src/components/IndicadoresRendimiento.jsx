...
import { Save, HardDrive, Server, Zap, Timer, RotateCcw, Play, Info } from 'lucide-react';
...
export default function IndicadoresRendimiento() {
  ...
  const [simKey, setSimKey] = useState(0); // Clave maestra para forzar reinicio instantáneo
  
  const timeoutsRef = useRef([]); // Referencia para guardar y limpiar los timers

  const startBenchmark = () => {
    if (isRunning) return;

    // Limpiar temporizadores previos por seguridad
    ...
    setFinished({});
    setIsRunning(false);
    setSimKey(prev => prev + 1); // Destruye y recrea las barras en 0%

    // Pequeño retraso para permitir que React renderice las barras en 0
    const t0 = setTimeout(() => {
      setIsRunning(true);
  ...
  const resetBenchmark = () => {
    // Matar todos los temporizadores en curso
    timeoutsRef.current.forEach(clearTimeout);
    timeoutsRef.current = [];
    
    setIsRunning(false);
    setFinished({});
    setSimKey(prev => prev + 1); // Forzar que las barras vuelvan a 0% sin animación hacia atrás
  };
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-[#141c25] border border-slate-800 p-6 rounded-xl shadow-lg">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 max-w-xl">
            Esta animación utiliza tiempos simulados a escala para visualizar la abismal diferencia que existe en la latencia al intentar acceder a un solo bloque de datos[cite: 9].
          </p>
        </div>
          ...
          <div key={dev.id} className="flex flex-col gap-1.5">
            ...
            <div className="flex justify-between items-end mb-1">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-sm font-bold ${dev.color}`}>{dev.name}</span>
              </div>
              <span className={`text-xs font-mono font-bold ${finished[dev.id] ? dev.color : 'text-slate-500'}`}>
                {finished[dev.id] ? dev.realTime : isRunning ? 'Buscando dato...' : 'En espera'}
              </span>
            </div>
            ...
            <div className="w-full h-4 bg-[#0b1016] rounded-full border border-slate-800 overflow-hidden shadow-inner">
              <motion.div
                ...
                initial={{ width: "0%" }}
                animate={{ width: isRunning || finished[dev.id] ? "100%" : "0%" }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-2">
        <div className="bg-[#0b1016] border-l-4 border-l-cyan-500 border border-slate-800 p-6 rounded-xl shadow-lg">
          ...
          <p className="text-sm text-slate-300 leading-relaxed text-justify mb-3">
            El <strong>Disquete</strong> y el <strong>HDD</strong> sufren de un cuello de botella físico imperdonable: dependen de motores giratorios y brazos lectores mecánicos. 
          </p>
          <p className="text-xs text-slate-400 bg-[#141c25] p-3 rounded border border-slate-700">
            Un HDD moderno (10ms) es 10 veces más rápido que un disquete antiguo (100ms), pero sigue siendo una eternidad para la CPU.
          </p>
        </div>
        ...
        <div className="bg-[#0b1016] border-l-4 border-l-blue-500 border border-slate-800 p-6 rounded-xl shadow-lg">
          ...
          <p className="text-sm text-slate-300 leading-relaxed text-justify mb-3">
            El <strong>SSD SATA</strong> elimina la mecánica reduciendo el tiempo a microsegundos, pero choca con la limitación del cable SATA. El <strong>SSD NVMe</strong> va directo al procesador vía PCIe.
          </p>
          <p className="text-xs text-slate-400 bg-[#141c25] p-3 rounded border border-slate-700">
            El NVMe (0.02ms) es 500 veces más rápido buscando un dato que un HDD mecánico[cite: 9]. ¡La información fluye casi a la velocidad de la RAM!
          </p>
        </div>
      </div>
